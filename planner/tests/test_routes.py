...
async def mock_event() -> Event:
    new_event = Event(
        ...
        title="FastAPI",
        image="https://linktomymage.com/image.png",
...
async def test_get_events(default_client: httpx.AsyncClient, mock_event: Event) -> None:
    response = await default_client.get("/event/d")
...
async def test_get_event(default_client: httpx.AsyncClient, mock_event: Event) -> None:
    url = f"/event/d/{str(mock_event.id)}"
...
async def test_post_event(default_client: httpx.AsyncClient, access_token: str) -> None:
    payload = {
        "title": "FastAPI123",
        "image": "https://linktomymage.com/image23.png",
        "description": "We will be discussing the contents234",
        "tags": ["python", "fastapi", "book"],
        "location": "Google Meet"
    }
    headers = {
    ...
    response = await default_client.post("/event/d/new", json=payload, headers=headers)
    assert response.status_code == 200
